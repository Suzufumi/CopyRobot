...
//		Dev->InitializeStates();
	...
	struct SmDrawObject :public DrawObjectBase {
		...
		weak_ptr<TextureResource> m_TextureResource;
		///光るライン用テクスチャ
		weak_ptr<TextureResource> m_LineTextureResource;
		///エミッシブ色
		bsm::Col4 m_Emissive;
		/// デフューズ色
		bsm::Col4 m_Diffuse;
	...
	void SmBaseDraw::SetConstants(SimpleConstants& SmCb, const MeshPrimData& data) {
		...
		Mat4x4 mvp = World * ViewMat * ProjMat;
		SmCb.InvMatrix = mvp.inverse();
		...
		SmCb.EyePos.w = 1.0f;
		SmCb.EyeDir = CameraPtr->GetAt() - CameraPtr->GetEye();
		SmCb.EyeDir.normalize();
		SmCb.EyeDir.w = 1.0f;

		//ラインテクスチャの存在
		if (GetLineTextureResource())
		{
			SmCb.ActiveFlg.y = 1;
		}
		else
		...
		if (pImpl->m_SmDrawObject.m_IsLighting)
			SmCb.ActiveFlg.z = 1;
		else
			SmCb.ActiveFlg.z = 0;
	...
	void SmBaseDraw::SetLineTextureResource(const shared_ptr<TextureResource>& TextureRes) {
		pImpl->m_SmDrawObject.m_LineTextureResource = TextureRes;
	}

	void SmBaseDraw::SetLineTextureResource(const wstring& TextureKey) {
		this->SetLineTextureResource(App::GetApp()->GetResource<TextureResource>(TextureKey));
	}
	...
	shared_ptr<TextureResource> SmBaseDraw::GetLineTextureResource() const {
		auto shptr = pImpl->m_SmDrawObject.m_LineTextureResource.lock();
		if (shptr) {
			return shptr;
		}
		return nullptr;
	...
	void SmBaseDraw::SetLightingActive(bool b) {
		pImpl->m_SmDrawObject.m_IsLighting = b;
	}
	...
	void PNTStaticDraw::OnCreate() {
		//ライティングのみだと極端になるので調整
		SetEmissive(bsm::Col4(0.4f, 0.4f, 0.4f, 0.0f));
		SetDiffuse(bsm::Col4(0.3f, 0.4f, 0.3f, 1.0f));
		SetSpecular(bsm::Col4(0.2f, 0.4f, 0.2f, 7.0f));
	}
	...
	void PNTStaticInstanceDraw::OnCreate() {
		//ライティングのみだと極端になるので調整
		SetEmissive(bsm::Col4(0.5f, 0.5f, 0.4f, 0.0f));
		SetDiffuse(bsm::Col4(0.4f, 0.4f, 0.4f, 1.0f));
		SetSpecular(bsm::Col4(1.0f, 1.0f, 1.0f, 50.0f));
		//行列バッファの作成
		CreateMatrixBuffer();
	}
