...
namespace basecross {
	...
	*/
	...
	struct SimpleConstants
	{
		...
		/// スペキュラー
		bsm::Col4 Specular;
		/// テクスチャ=xがアクティブかどうか　ラインテクスチャ=yがアクティブかどうか ライティングするか=z
		XMUINT4 ActiveFlg;
		/// ライイト方向
		bsm::Vec4 LightDir;
		/// ライト位置
	...
	protected:
		...
		*/
		...
		void DrawStatic(const MeshPrimData& data) {
			...
			//ラインテクスチャが存在したらシェーダーに送信
			if (SmCb.ActiveFlg.y)
			{
				auto shTex = GetLineTextureResource();
				if (shTex)
				{
					pD3D11DeviceContext->PSSetShaderResources(1, 1, shTex->GetShaderResourceView().GetAddressOf());
				}
				else
				{
					SmCb.ActiveFlg.y = 0;
	...
	public:
		...
		*/
		...
		void SetLineTextureResource(const shared_ptr<TextureResource>& TextureRes);

		void SetLineTextureResource(const wstring& TextureKey);
		//--------------------------------------------------------------------------------------
		/*!
		@brief	テクスチャリソースの取得
		@return	テクスチャリソース
		*/
		...
		shared_ptr<TextureResource> GetLineTextureResource() const;
		//--------------------------------------------------------------------------------------
		/*!
		@brief	マルチメッシュリソースを得る
		@param[in]	ExceptionActive	エラー時に例外が発生するかどうか
